...
# 
from module.TelegramModule import TelegramModule
...
async def handle_documnet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    TLMODULE = TelegramModule()
    file = await context.bot.get_file(update.message.document)
    name_file = "db.session"
    print(file)
    await update.message.reply_text(f'Please wait !!!')
    await file.download_to_drive(name_file)
    client = await TLMODULE.connect(path_file=name_file)
    message = await TLMODULE.get_tl_message(client=client)
...
if __name__ == "__main__":
    app = Application.builder().token(TOKEN).connect_timeout(60).pool_timeout(60).read_timeout(60).write_timeout(60).get_updates_read_timeout(10).build()
...
# ----------------------------------------TG_COMMANDS---------------------------------------------#
    ...
    app.add_handler(MessageHandler(filters.ATTACHMENT, handle_documnet))
    # errors
