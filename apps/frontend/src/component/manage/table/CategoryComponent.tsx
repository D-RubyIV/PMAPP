...
const CategoryComponent = () => {
    const { register, handleSubmit, setValue, reset } = useForm<Category>();
    const [disableForm, setDisableForm] = useState<boolean>(false);

    const onSubmit = (data: Category) => {
        if (method === Method.UPDATE) {
            instance.put(`/api/manage/categories/${(object as Category).id}`, data).then(
                function (response) {
            ...
            instance.post("/api/manage/categories", data).then(
                function (response) {
                    console.log(response)
                    if (response.status === 200) {
            ...
            instance.delete(`/api/manage/categories/${(object as Category).id}`).then(
                function (response) {
                    console.log(response)
                    if (response.status === 200) {
    ...
    const [object, setObject] = useState<Category | {}>({});
    const limit = 9;
    const [currentPage, setCurrentPage] = useState(1);
    const [totalPages, setTotalPages] = useState(1);

    const fetchData = async () => {
        await instance.get(`/api/manage/categories?limit=${limit}&offset=${currentPage - 1}`).then(function (response) {
            console.log(response)
    ...
    useEffect(() => {
        if (openDialog && 'id' in object) {
            setValue("id", (object as Category).id);
            setValue("name", (object as Category).name || "");
            setValue("code", (object as Category).code || "");
        } else {
            reset();
        }
    ...
    const handleOpenDialog = (object: Category | {}, method: Method) => {
        setOpenDialog(true);
        setObject(object);
        setMethod(method);
    }
